...
class zzhuzhu:
    ...
    def getbbscontent(self,id,showcontent=""):
        bbscontent=cache.get("app_bbspostcontent"+str(id))
        bbscontent=None
        if (bbscontent==None):
            ...
            if alist:
                ...
                cache.set("app_bbspostcontent"+str(id),bbscontent,60)
        return bbscontent
    #置顶信息
    ...
    def isnotice(self,content_id,company_id,type):
        if company_id:
            ...
            return 1
    #朋友圈列表
    def getquanlist(self,company_id,frompageCount,limitNum,qtype=""):
        if qtype=="all":
            sql="select a.id,a.company_id from bbs_post as a where a.check_status=1 and a.is_del=0 order by a.gmt_created desc limit %s,%s"
            result=self.dbc.fetchalldb(sql,[frompageCount,limitNum])
        elif qtype=="hot":
            sql="select a.id,a.company_id from bbs_post as a where a.check_status=1 and a.is_del=0 and DATEDIFF(CURDATE(),a.reply_time)<=7 order by a.visited_count desc limit %s,%s"
            result=self.dbc.fetchalldb(sql,[frompageCount,limitNum])
        else:
            sql="select a.id,a.company_id from bbs_post as a inner join company_addressbook as b on a.company_id=b.forcompany_id and (b.company_id=%s or a.company_id=%s) and a.check_status=1 and a.is_del=0 order by a.gmt_created desc limit %s,%s"
