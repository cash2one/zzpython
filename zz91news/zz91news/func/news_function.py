...
#            keywords=keywords.replace('%','%%')
    ...
    def myfavorite(self,company_id,frompageCount="",limitNum=""):
        sql="select content_id from myfavorite where company_id=%s and favorite_type_code=%s order by gmt_created desc limit %s,%s"
        mlist=dbc.fetchalldb(sql,[company_id,'10091012',frompageCount,limitNum])
        listall=[]
        for list in mlist:
            id=list[0]
    ...
    def gettypelist(self,frompageCount,limitNum,reid='',keywords='',has_news='',fromnews='',has_txt='',typeid='',typeid2=''):
        ...
        sql+=' order by sortrank,id limit %s,%s'
...
#                pubdate=int_to_str(intdate)
    ...
    def getzhlist(self,frompageCount,limitNum,searchlist=""):
        ...
        if searchlist.has_key("noexpress"):
            sql+=' order by a.start_time asc limit %s,%s'
        else:
            sql+=' order by a.start_time desc limit %s,%s'
