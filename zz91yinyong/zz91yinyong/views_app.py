...
def getvcardpic(request,company_id):
    ...
    if returnresult:
        ...
        if domain_zz91:
            ...
            domain="http://mobile.m.zz91.com/mobile/index"+str(company_id)+".htm"
    else:
        #domain="http://m.zz91.com/company/detail"+str(company_id)+".html"
        domain="http://mobile.m.zz91.com/mobile/index"+str(company_id)+".htm"
    """
    domain="http://m.zz91.com/company/card"+str(company_id)+".html"
    if domain:
        qr = qrcode.QRCode(
                        version=1,
                        error_correction=qrcode.constants.ERROR_CORRECT_L,
...
def getdomainpic(request,company_id):
    ...
    if returnresult:
        ...
        domain=None
    '''
    domain="http://www.zz91.com/m/"+str(company_id)+".html"
    if domain:
        qr = qrcode.QRCode(
                        version=1,
                        error_correction=qrcode.constants.ERROR_CORRECT_L,
...
def javagetnewslist_json(request):
    ...
    newslist=getnewslist(keywords=keywords,frompageCount=0,typeid=typeid,typeid2=typeid2,limitNum=int(num),pic=pic)
...
def qrcodeimg(request):
    ...
    if not type:
        return HttpResponse("err")
    if type=="trade":
        arg="http://m.zz91.com/trade/detail"+str(id)+".html"
    if type=="price":
        arg="http://m.zz91.com/priceviews/?id="+str(id)
    if type=="company":
        arg="http://m.zz91.com/company/shop"+str(id)+".html"
    qr = qrcode.QRCode(
        version=1,
        error_correction=qrcode.constants.ERROR_CORRECT_L,
        box_size=4,
...
def choujiangstaus(request):
    ...
    s=getchoujiangstaus(company_id,usertoken=usertoken)
    if s:
        return HttpResponse('var _suggest_result_='+str(s))
#获得抽奖次数
def choujiangcount(request):
    company_id=request.GET.get("company_id")
    sql="select sum(bnum) from subject_choujiang where company_id=%s and btype=8"
...
#获得国庆2016抽奖次数
def guoqincount2016(request):
    company_id=request.GET.get("company_id")
    sql="select id,jiangpin from subject_choujiang where company_id=%s and btype=6"
    result=dbc.fetchonedb(sql,[company_id])
    if result:
        if (str(result[1])!="9"):
            return HttpResponse('var _suggest_result_={"count":"-1"}')
        return HttpResponse('var _suggest_result_={"count":"1"}')
    else:
        return HttpResponse('var _suggest_result_={"count":"0"}')
#国庆2016抽奖
def guoqin2016staus(request):
    company_id=request.GET.get("company_id")
    s=getguoqin2016staus(company_id)
    if s:
        return HttpResponse('var _suggest_result_='+str(s))
#2016砸金蛋活动
...
def subject_hongbao(request):
    ...
    if company_id:
        ...
        if datatype=="json":
            jsonlist={'hongbao':jiangpin,'checked':checked}
            return HttpResponse(simplejson.dumps(jsonlist, ensure_ascii=False))
        else:
            return HttpResponse('var _suggest_result_={"hongbao":"'+str(jiangpin)+'","checked":'+str(checked)+'}')
    else:
        jsonlist={'err':'true'}
        return HttpResponse(simplejson.dumps(jsonlist, ensure_ascii=False))
#非法字符
