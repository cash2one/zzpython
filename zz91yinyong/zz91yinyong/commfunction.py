...
def taizhou2016(company_id=""):
	...
	return None
#判断是否可以抽奖（完成3个操作的用户）
def getchoujiangstaus(company_id,usertoken=""):
	gmt_created=getNow()
	#taizhou2016(company_id=company_id)
	bnum=-1
	btype=8
	sql="select id from subject_choujiang where company_id=%s and btype=%s"
	result=dbc.fetchonedb(sql,[company_id,btype])
	...
	alllist=[]
	h0=range(10,20)+[0,3]
	h1=range(250,254)+[1,3]
	h6=range(20,33)+[6,3]
	h5=range(60,61)+[3]
	h4=range(140,143)+[4,3]
	h3=range(150,151)+[3]
	h2=range(190,192)+[3]
	h7=range(250,260)+[7,3]
	...
	haoma=[0,1,2,3,4,5,6,7]
	hlist=alllist[random.choice(range(0,7))]
	#实际中奖个数
	limitcount=[0,5,3,6,15,6,7,10]
	...
	#设置中奖个数
	if xuanzhong>0:
		sql="select count(0) from subject_choujiang where jiangpin=%s and btype=%s"
		result=dbc.fetchonedb(sql,[xuanzhong,btype])
		if result:
			if result[0]>=limitcount[xuanzhong]:
				list["xuanzhong"]=0
	"""
	#获得抽奖次数
	sql="select sum(bnum) from subject_choujiang where company_id=%s and btype=%s"
	result=dbc.fetchonedb(sql,[company_id,btype])
	if result:
		if result[0]:
			choujiangcount=result[0]
			list["choujiangcount"]=str(int(choujiangcount))
	if choujiangcount>0:
		#保存抽中记录
		jiangpin=list["xuanzhong"]
		nowdate=str(datetime.datetime.now())
		
		if jiangpin==3 or jiangpin==0 or jiangpin==4:
			if jiangpin==3:
				money="50"
			if jiangpin==0:
				money="300"
			...
			payload={'company_id':str(company_id),'ftype':'53','fee':money,'more':'1','usertoken':str(usertoken)}
			r= requests.get("http://apptest.zz91.com/sendfee.html",params=payload)
			list['err']=str(r)
		list['jiangpin']=jiangpin
		
		#显示联系方式
		if jiangpin==2:
			payload={'company_id':str(company_id),'usertoken':usertoken,'opentype':'showcontact','datevalue':'300'}
			r= requests.get("http://apptest.zz91.com/qianbao/openservice.html",params=payload)
		#显示联系方式
		if jiangpin==7:
			payload={'company_id':str(company_id),'usertoken':usertoken,'opentype':'proreflush','datevalue':'1'}
			r= requests.get("http://apptest.zz91.com/qianbao/openservice.html",params=payload)
...
def getzajindanstaus(company_id):
	...
	try:
		...
		r= requests.get("http://apptest.zz91.com/qianbao/qianbaobaoblance.html",params=payload)
		if r:
	...
	if float(qianbaobaoblance)>10:
		...
		if choujiangcount<3:
			...
			try:
				...
				r= requests.get("http://apptest.zz91.com/payfee.html",params=payload)
				sql="insert into subject_choujiang(btype,gmt_created,bnum,company_id,jiangpin) values(%s,%s,%s,%s,%s)"
				dbc.updatetodb(sql,[btype,gmt_created,bnum,company_id,jiangpin])
				list["choujiangcount"]=str(int(choujiangcount)-1)
			except:
...
#2016国庆抽奖
def getguoqin2016staus(company_id):
	gmt_created=getNow()
	bnum=1
	btype=6
	list={"choujiangcount":"0"}

	if not company_id and str(company_id)!="0":
		return list
	#中奖号码
	alllist=[]
	h1=[0]*1
	...
	h4=[3]*4
	h5=[4]*1
	alllist=h1+h2+h3+h4+h5
	
	hlist=random.choice(alllist)
	
	
	list["list"]=alllist
	xuanzhong=random.choice(alllist)
	list["xuanzhong"]=xuanzhong
	choujiangcount=0
	#获得抽奖次数
	sql="select id from subject_choujiang where company_id=%s and btype=6 and jiangpin=9"
	result=dbc.fetchonedb(sql,[company_id])
	if result:
		#保存抽中记录
		jiangpin=list["xuanzhong"]
		sql="update subject_choujiang set jiangpin=%s where id=%s"
		dbc.updatetodb(sql,[jiangpin,result[0]])
		list["choujiangcount"]="1"
