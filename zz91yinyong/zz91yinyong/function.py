...
#----新闻列表 翻页
def getnewslist(keywords="",frompageCount="",limitNum="",typeid="",allnum="",typeid2="",contentflag="",cursornews="",pic=""):
	port = spconfig['port']
	cl = SphinxClient()
	...
	if typeid2:
		cl.SetFilter('typeid2',[int(typeid2)])
	...
	searstr=''
	if pic or "p" == keywords:
		searstr='@(flag) "p"'
	if (keywords):
		searstr+=' @(title,keywords) '+keywords
	res = cl.Query (searstr,'news')
	listall_news=[]
	listcount_news=0
	if res:
		if res.has_key('matches'):
			...
			for match in tagslist:
				...
				imgurl=''
				if not litpic:
					imgurl=get_img_url(content)
					if imgurl:
						litpic=imgurl[0]
				if litpic:
					if "zz91.com" not in litpic:
				...
				list1={'title':title,'title10':title10,'id':id,'pubdate':pubdate2,'littlecontent':littlecontent,'content':content,'havepic':havepic,'newsurl':newsurl,'weburl':weburl,'mobileweburl':mobileweburl,'litpic':litpic}
				if not contentflag:
					list1['content']=None
				listall_news.append(list1)
			listcount_news=res['total_found']
