...
def getproductsinfo(pdtid,cursor,keywords):
	...
	if productlist:
		...
		#来电宝客户
		if (productlist[0]):
			sqll="select id from crm_company_service where company_id=%s and crm_service_code in(1007,1008) and apply_status=1"
			cursor.execute(sqll,[productlist[0]])
			ldbresult=cursor.fetchone()
			if ldbresult:
				sqlg="select front_tel from phone where company_id=%s"
				cursor.execute(sqlg,[productlist[0]])
		...
		if (area_code):
			...
			cursor.execute(sqld,[area_code[:-4]])
		...
		cursor.execute(sql1,[productlist[2]])
