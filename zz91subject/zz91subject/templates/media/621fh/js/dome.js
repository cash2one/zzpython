...
var MouseDirection = function (element, opts) {
    ...
    var calculate = function (element, e) {
        ...
        var width_h,height_h;

        if (!rect.width) {
            width_h = rect.right - rect.left;
        }
        width_h = rect.right - rect.left;
        if (!rect.height) {
            height_h = rect.bottom - rect.top;
        }
        height_h = rect.bottom - rect.top;
        ...
        x4 = rect.left + width_h;
        y4 = -(rect.top + height_h);

        x0 = rect.left + width_h / 2;
        y0 = -(rect.top + height_h / 2);
