		...
		if params.has_key('echostr'):
			...
			if request.body:
				...
				if (content==None  and EventKey=='toupiao'):
					titlelist=[{'title':'ZZ91再生网金牌供应商评选活动,点此投票','Description':'ZZ91再生网2015-2016年年度金牌供应商评选活动','PicUrl':'http://img0.zz91.com/zz91/weixin/images/11111111.gif','Url':'http://m.zz91.com/wechat/auth_login.html?clientid='+toUserName+''}]
					ArticleCount=len(titlelist)
					xml=backxml(xmltype=3,fromUserName=toUserName,toUserName=fromUserName,ArticleCount=ArticleCount,titlelist=titlelist)
					return HttpResponse(xml)
				if (content==None  and EventKey=='binding'):
				...
				if (Event1 == "subscribe" or Event1 == "scan"):
					...
					try:
						...
						#投票
						if (title=="投票"):
							titlelist=[{'title':'ZZ91再生网2015-2016年年度金牌供应商评选活动','Description':'ZZ91再生网年度金牌供应商评选活动','PicUrl':'http://img0.zz91.com/zz91/weixin/images/11111111.gif','Url':'http://m.zz91.com/wechat/auth_login.html?clientid='+toUserName+''}]
							ArticleCount=len(titlelist)
							xml=backxml(xmltype=3,fromUserName=toUserName,toUserName=fromUserName,ArticleCount=ArticleCount,titlelist=titlelist)
							return HttpResponse(xml)
						if (title[0:1]=="t" or title[0:1]=="T"):
							orderid=title.replace("t","")
							orderid=orderid.replace("T","")
							orderid=orderid.replace("+","")
							orderid=orderid.replace(" ","")
							return votecom(orderid,toUserName,fromUserName)
						if (title=="支付"):
							title="支付方式\n\nZZ91支付宝帐号：zhifu@zz91.com\n\n开户银行：中国农业银行杭州朝晖支行\n收 款 人：杭州阿思拓信息科技有限公司\n帐　　号：19-0156 0104 0013 383\n\n公司地址：浙江省杭州市江干区九盛路9号东方电子商务园13栋\n\n邮编：310019 \n\n传真：0571-56637777\n"
							xml=backxml(xmltype=2,fromUserName=toUserName,toUserName=fromUserName,title=title)
							return HttpResponse(xml)
...
def votecom(orderid,toUserName,fromUserName):
	...
	company_id=None
	if orderid:
		sql="select company_id,company_name from vote_list where orderid=%s"
		result=dbc.fetchonedb(sql,[orderid])
		if result:
			company_id=result[0]
			...
			if vcount>=3:
				...
				if company_id:
					...
					if result:
						...
						title="恭喜您，您已投票成功，您投的公司是："+company_name+"，目前票数："+str(vcount)+",<a href='http://m.zz91.com/wechat/auth_login.html?clientid="+toUserName+"'>点此查看详情</a>"
						xml=backxml(xmltype=2,fromUserName=toUserName,toUserName=fromUserName,title=title)
						return HttpResponse(xml)
	title="系统错误，请重试！"
	xml=backxml(xmltype=2,fromUserName=toUserName,toUserName=fromUserName,title=title)
