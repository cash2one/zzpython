...
import MySQLdb,sys,os,memcache,settings,urllib,re,time,requests,simplejson
...
from trade import getcompinfo as getproinfo
...
def index(request):
    ...
    if not type:
        type='price'
    return render_to_response('aui/jiage/category.html',locals())
...
def pricelist(request,category_id='',assist_id='',pinyin='',apinyin='',page=''):
    ...
    if orderflag:
        ...
        keywordslist=""
        if not orderlist:
            return HttpResponseRedirect("/order/price.html")
        if orderlist:
    ...
    if (keywords):
        ...
        webtitle=keywords+webtitle
        #webtitle+="-"+keywords+"报价"
    ...
    if not keywords:
        keywords=""
    return render_to_response('aui/jiage/list.html',locals())
...
#            searchkeyword=assist_label
    ...
    favoriteflag=0
    if mycompany_id:
        favoriteflag=isfavorite(id,'10091004',mycompany_id)
    type_id=''
...
#                searchkeyword='废塑料'
    ...
    return render_to_response('aui/jiage/view.html',locals())
...
def pricechartdata(request):
    ...
    chartpricelist=zzprice.getchartpricelist(keywords=label,frompageCount=0,gmt_begin=gmt_begin,gmt_end=gmt_end,area=area,group='postdate',categoryid=type_id)
    if not chartpricelist:
        chartpricelist=zzprice.getchartpricelist(keywords=label,frompageCount=0,gmt_begin=gmt_begin,gmt_end=gmt_end,area=area,group='postdate')
    if chartpricelist:
