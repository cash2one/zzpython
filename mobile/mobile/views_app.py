...
def zz91pay(request):
    ...
    #不用登录的情况下支付
    nologin=request.POST.get('nologin')
    ...
    payflag=request.POST.get("payflag")
    ...
    valu=[out_trade_no,subject,relfee,contact,mobile,is_success,payreturn_url,paytype,company_id,gmt_created,buyer_id]
    sql="insert into pay_order(out_trade_no,subject,total_fee,contact,mobile,is_success,payreturn_url,paytype,company_id,gmt_created,buyer_id) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
    dbc.updatetodb(sql,valu)
    payload={'order_id':out_trade_no,'identity_id':out_trade_no,'product_name':subject,'product_desc':'','amount':total_fee,'merchant_url':merchant_url,'user_ip':user_ip,'pagefrom':pagefrom}
    url="/pingxx/pay.html?"+urllib.urlencode(payload)
    return HttpResponseRedirect(url)
...
def callback_post(request):
    ...
    hmac_safe=request.GET.get("hmac_safe")
    if hmac_safe:
        out_trade_no=r6_Order=request.GET.get("r6_Order")
        trade_no=r2_TrxId=request.GET.get("r2_TrxId")
        trade_status=r1_Code=request.GET.get("r1_Code")
        if str(trade_status)=="1":
            sql="update pay_order set is_success=%s,trade_no=%s where out_trade_no=%s"
            dbc.updatetodb(sql,['SUCCESS',trade_no,out_trade_no])
            javaurl=urllib.urlencode(request.GET)
            #return HttpResponse("http://10.171.204.51:8030/yb/callback.jsp?"+javaurl)
            requests.get("http://10.171.204.51:8030/yb/callback.jsp?"+javaurl)
    ...
    sign=resualt['sign']
    sql="insert into pingxx (content) values(%s)"
    dbc.updatetodb(sql,[str(resualt)])
    if (trade_status==1):
        sql="update pay_order set is_success=%s,trade_no=%s,notify_id=%s where out_trade_no=%s"
        dbc.updatetodb(sql,['SUCCESS',trade_no,sign,out_trade_no])
        ...
        if result:
            ...
            company_id=result[3]
            #淘再生跳转业务处理
            if paytype=="taozaisheng-pay":
                #javaurl=urllib.urlencode(resualt)
                requests.get("http://10.171.204.51:8030/yb/callback.jsp",params=resualt)
                return HttpResponse("SUCCESS")
...
def callback_get(request):
    ...
    if (trade_status==1):
        ...
        if result:
            ...
            if paytype=="taozaisheng-pay":
                #javaurl=urllib.urlencode(request.GET)
                r=requests.get("http://10.171.204.51:8030/yb/callback.jsp",params=resualt)
                r.encoding = 'utf-8'
                if r.text=="SUCCESS":
...
def zz91payverify_notify(request):
    ...
    sql="insert into pingxx (content) values(%s)"
    dbc.updatetodb(sql,[str(request.body)])
    #return HttpResponse("SUCCESS")
    wx=0
    total_fee=None
    if not out_trade_no:
        ...
        if not out_trade_no:
            ...
            out_trade_no = xml.find("out_trade_no").text
            trade_no= xml.find("transaction_id").text
            trade_status=xml.find("result_code").text
            sign=xml.find("sign").text
            total_fee=xml.find("total_fee").text
            wx=1
    if trade_status in ("TRADE_FINISHED","TRADE_SUCCESS","SUCCESS"):
        sql="select payreturn_url,paytype,total_fee,company_id,is_success from pay_order where out_trade_no=%s"
        result=dbc.fetchonedb(sql,[out_trade_no])
        if result:
            payreturn_url=result[0]
            paytype=result[1]
            fee=result[2]
            if total_fee:
                fee=float(total_fee)/100
            company_id=result[3]
            is_success=result[4]
            #如果已经支付成功直接返回成功
            if is_success in ("TRADE_FINISHED","TRADE_SUCCESS","SUCCESS","1"):
                if (wx==1):
                    ret_dict = {
                        'return_code': 'SUCCESS',
                        'return_msg': 'OK',
            ...
            if str(trade_no)==str(out_trade_no):
                a=1
            else:
                sql="update pay_order set is_success=%s,trade_no=%s where out_trade_no=%s"
        ...
        return HttpResponse("FAIL")
#支付宝支付完成通知，异步返回通知
def zz91payverify_notifyali(request):
    out_trade_no=request.POST.get("out_trade_no")
    trade_no=request.POST.get("trade_no")
    trade_status=request.POST.get("trade_status")
    sql="insert into pingxx (content) values(%s)"
...
def paysave(paytype,trade_no,company_id,fee):
    if paytype=="qianbao":
        ...
        if result:
            ...
            sql="update pay_order set is_success=%s where out_trade_no=%s"
            dbc.updatetodb(sql,["SUCCESS",trade_no])
            #充值活动  2017开春活动
            sql="select id,bnum,jiangpin from subject_choujiang where company_id=%s and btype='20111009' and bnum=0"
            result=dbc.fetchonedb(sql,[company_id])
            if result:
                fee=result[2]
                sid=result[0]
                zzq.getsendfee(company_id,fee,"57")
                sqld="update subject_choujiang set bnum=%s where id=%s"
                dbc.updatetodb(sqld,[1,sid])
            """
            #----(充值做活动)
    ...
    if paytype=="shuang11chongzhi":
        ...
        if result:
            ...
            if fee==1500:
                cfee=600
            zzq.insert_voucher(title=title,company_id=company_id,qtype_id=8,fee=cfee)
    ...
    #双11砍价活动
    if paytype=="shuang11":
        sql="select buyer_id from pay_order where out_trade_no=%s"
        result=dbc.fetchonedb(sql,[trade_no])
        if result:
            buyer_id=result[0]
            sqlc="update subject_kanjia_baoming set payflag=1 where id=%s"
            dbc.updatetodb(sqlc,[buyer_id])
            sql="update pay_order set is_success=%s where out_trade_no=%s"
            dbc.updatetodb(sql,["SUCCESS",trade_no])
            sqlc="select pro_id from subject_kanjia_baoming where id=%s"
            resultc=dbc.fetchonedb(sqlc,[buyer_id])
            if resultc:
                pro_id=resultc[0]
                #显示联系方式
                if str(pro_id)=="2":
                ...
                if str(pro_id)=="4":
                    #充值
                    zzq.getsendfee(company_id,fee,"5",more=1)
                    #砍价值
                    zzq.getsendfee(company_id,100-int(fee),"8",more=1)
                if str(pro_id)=="5":
                    #充值
                    zzq.getsendfee(company_id,fee,"5",more=1)
                    #砍价值
                    zzq.getsendfee(company_id,300-int(fee),"8",more=1)
    #淘再生订单支付
    if paytype=="taozaisheng-pay":
        sql="update pay_order set is_success=%s where out_trade_no=%s"
        dbc.updatetodb(sql,["SUCCESS",trade_no])
        
        #sql="update orders set status=2 where order_no=%s"
        #dbtzs.updatetodb(sql,[trade_no])
    #621再生汇汇费支付
    if paytype == "621zsh":
        sql="select buyer_id from pay_order where out_trade_no=%s"
        result=dbc.fetchonedb(sql,[trade_no])
        if result:
            buyer_id=result[0]
            sqlc="update zsh_list set ispay=1 where id=%s"
            dbc.updatetodb(sqlc,[buyer_id])
            sql="update pay_order set is_success=%s where out_trade_no=%s"
            dbc.updatetodb(sql,["SUCCESS",trade_no])
    #废纸展会团购
    if paytype=='groupbuy':
        ...
        if result:
            buyer_id=result[0]
            sqlc="update subject_groupbuy_join set ispay=1 where id=%s"
            dbc.updatetodb(sqlc,[buyer_id])
            sql="update pay_order set is_success=%s where out_trade_no=%s"
            dbc.updatetodb(sql,["SUCCESS",trade_no])
...
def zz91payreturn_url(request):
    ...
    if result=="success":
        ...
        return HttpResponse(err)
#更多服务
def servicemore(request):
    return HttpResponsePermanentRedirect("http://m.zz91.com/")
    return render_to_response('service/servicemore.html',locals())
