...
class Client(object):
    ...
    def token(self):
        now = time.time()
        weixintoken=cache.get("weixintoken"+self.appid)
        weixintime=cache.get("weixintime"+self.appid)
        if weixintoken and weixintime:
            if int(now)-int(weixintime) < 100:
                return weixintoken
        json = self.grant_token()
        cache.set("weixintoken"+self.appid,json["access_token"],100)
        cache.set("weixintime"+self.appid,now,100)
    ...
    def send_template_message(self, datava):
        """
        发送模板消息
        详情请参考 https://mp.weixin.qq.com/advanced/tmplmsg?action=faq&token=1241840219&lang=zh_CN
    ...
    def send_article_message(self, user_id, articles):
        ...
        """
        datava=json.dumps({"touser": str(user_id),"msgtype": "news","news": {"articles": articles_data}},ensure_ascii=False,indent=2)
        return self.post(
            url="https://api.weixin.qq.com/cgi-bin/message/custom/send",
            #params="{\"touser\":\""+str(user_id)+"\",\"msgtype\":\"text\",\"text\":{\"content\":\""+content+"\"}}" 
            data=datava,
    ...
    def show_qrcode(self, ticket):
        ...
        return requests.get(
            ...
            params=json.dumps({"ticket": ticket},ensure_ascii=False,indent=2),
            headers = {'content-type': 'application/json; encoding=utf-8'}
        )
