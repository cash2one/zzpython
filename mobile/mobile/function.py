...
def getmyquestion(company_id,frompageCount,limitNum):
	...
	if result1:
		...
		count=0
	sql='select id,title,post_time,reply_count,content,company_id from bbs_post where company_id=%s  order by id desc limit %s,%s'
	...
	if resultlist:
		for result in resultlist:
			...
			faceurl=getfacepic(result[5])
			nickname=getusername(result[5])
			list={'id':result[0],'title':result[1],'post_time':formattime(result[2],0),'reply_count':result[3],'content':content,'faceurl':faceurl,'company_id':result[5],'nickname':nickname}
			listall.append(list)
...
def getpostviewed(company_id,bbs_post_id):
	if company_id:
		...
		if resultlist:
			...
			return "1"
#查看为查看的回复数
def getnoviewreplycount(company_id):
	#获取未读回复列表
	sql="select max_reply_id,viewlist from bbs_post_reply_view where company_id=%s"
	result=dbc.fetchonedb(sql,[company_id])
	max_reply_id=0
	viewlist=''
	if result:
		max_reply_id=result[0]
		viewlist=result[1]
	if viewlist:
		...
		viewlist={}
	#更新未读列表
	maxreplyid=max_reply_id
	sql="select id from bbs_post_reply where tocompany_id=%s and id>%s order by id asc"
	result=dbc.fetchalldb(sql,[company_id,max_reply_id])
	for list in result:
		viewlist[str(list[0])]=0
		maxreplyid=list[0]
	if viewlist:
		sql="select id from bbs_post_reply_view where company_id=%s"
		result=dbc.fetchonedb(sql,[company_id])
		if result:
			sql="update bbs_post_reply_view set viewlist=%s,max_reply_id=%s where company_id=%s"
			dbc.updatetodb(sql,[str(viewlist),maxreplyid,company_id])
		else:
			sql="insert into bbs_post_reply_view(viewlist,max_reply_id,company_id) values(%s,%s,%s)"
			dbc.updatetodb(sql,[str(viewlist),maxreplyid,company_id])
	return len(viewlist)
...
def getmessgelist(company_id,frompageCount,limitNum):
	...
	sql="select count(0) from bbs_post_reply as a left join bbs_post as b on a.bbs_post_id=b.id where a.tocompany_id=%s and a.check_status='1'"
	result=dbc.fetchonedb(sql,[company_id])
	count=0
	if result:
		count=result[0]
		
	sql="select a.company_id,a.bbs_post_id,a.tocompany_id,a.content,a.gmt_created,b.content as bbstitle,a.id from bbs_post_reply as a left join bbs_post as b on a.bbs_post_id=b.id where a.tocompany_id=%s and a.check_status='1' order by a.id desc limit %s,%s"
	result=dbc.fetchalldb(sql,[company_id,frompageCount,limitNum])
	listall=[]
	if result:
		for list in result:
			company_id=list[0]
			post_id=list[1]
			tocompany_id=list[2]
			replycontent=filter_tags(list[3])
			reply_time=formattime(list[4],0)
			nickname=getusername(company_id)
			bbstitle=filter_tags(list[5])
			reply_id=list[6]
			faceurl=getfacepic(company_id)
			isview=getisviewreply(mycompany_id,reply_id)
			list={'id':reply_id,'replycontent':replycontent,'faceurl':faceurl,'nickname':nickname,'post_title':bbstitle,'company_id':company_id,'post_id':post_id,'reply_time':reply_time,'isview':isview}
			listall.append(list)
	return {'list':listall,'count':count,'notviewcount':notviewcount}
#回复是否已读 
def getisviewreply(company_id,reply_id):
...
def getmyreply(company_id,frompageCount,limitNum):
	sql1='select count(0) from bbs_post as a left join bbs_post_reply as b on a.id=b.bbs_post_id where b.company_id=%s'
	result1=dbc.fetchnumberdb(sql1,[company_id])
	if result1:
		count=result1
	else:
		count=0
	sql='select b.id as reply_id,b.content as replycontent,b.gmt_created,a.company_id,a.title,a.reply_count,a.id,a.content from bbs_post as a left join bbs_post_reply as b on a.id=b.bbs_post_id where b.company_id=%s order by b.gmt_created desc limit %s,%s'
	resultlist=dbc.fetchalldb(sql,[company_id,frompageCount,limitNum])
	listall=[]
	if resultlist:
		for result in resultlist:
			...
			replyconent=filter_tags(result[1])
			content=filter_tags(result[7])
			faceurl=getfacepic(company_id)
			list={'id':result[0],'post_id':result[6],'content':content,'replyconent':replyconent,'gmt_created':formattime(result[2],0),'nickname':nickname,'post_title':result[4],'reply_count':reply_count,'faceurl':faceurl,'company_id':company_id}
			listall.append(list)
	return {'list':listall,'count':count}
#我的关注
def getmygaunzhu(company_id,frompageCount,limitNum):
	...
	if resultlist:
		for result in resultlist:
			...
			resultd=dbc.fetchonedb(sqld,[result[0]])
			company_id=0
			nickname=''
			faceurl=''
			if resultd:
				company_id=resultd[0]
				nickname=getusername(resultd[0])
				faceurl=getfacepic(resultd[0])
			list={'bbs_post_id':result[0],'company_id':company_id,'nickname':nickname,'faceurl':faceurl,'title':result[1],'post_time':formattime(result[2],0)}
			listall.append(list)
	return {'list':listall,'count':count}
#----获得互助帖子标题
...
def getfacepic(company_id):
	...
	if (not result):
		...
		if result[0]:
			return 'http://img3.zz91.com/200x15000/'+result[0]
		else:
			return 'http://static.m.zz91.com/aui/images/noavatar.gif'
#邀请问答
		...
		if cid:
			...
			if result:
				for list in result:
					...
					result1=dbc.fetchalldb(sql1,[id])
					if result1:
...
def getbbslist(kname,frompageCount,limitNum,category_id='',fromtime='',endtime='',datetype='',htype='',bbs_post_assist_id=''):
	catelist=getbbs_categorys(category_id)
	port = spconfig['port']
	cl = SphinxClient()
	cl.SetServer ( spconfig['serverid'], port )
	cl.SetMatchMode ( SPH_MATCH_BOOLEAN )
			...
			for match in tagslist:
				...
				if (bbscontent==None):
					...
					if alist:
						...
						content=subString(filter_tags(alist[0]),250)
...
#----公司信息列表 翻页
def getcompanylist(kname,frompageCount,limitNum,ldb='',province='',jisd=''):
	port = spconfig['port']
	cl = SphinxClient()
	cl.SetServer ( spconfig['serverid'], port )
	cl.SetMatchMode ( SPH_MATCH_BOOLEAN )
	...
	if (kname or province):
		searchprovince=''
		if province:
			searchprovince+=' @(name,address,area_name,area_province) '+province
		if kname:
			searchprovince+=' @(name,business,address,sale_details,buy_details,tags,area_name,area_province) '+kname
		res = cl.Query (searchprovince,'company')
	else:
		searchprovince=''
		if jisd:
			searchprovince+=' @(name,address,area_name,area_province) '+jisd
		res = cl.Query (searchprovince,'company')
	listcount_comp=0
	listall_comp=[]
	if res:
...
def weixinbinding(weixinid):
	...
	if list:
		...
		return None
def weixinhavebind(account):
	sql="select open_id from oauth_access where target_account=%s and open_type='weixin.qq.com'"
	list=dbc.fetchonedb(sql,[account])
	if list:
		return list[0]
	else:
		return None
...
def getcompanyname(uname):
	...
	if (newcode):
		...
		if clist:
			return {'company_id':company_id,'companyname':clist[0],'faceurl':faceurl}
...
def getleavewordslist(frompageCount,limitNum,company_id,sendtype):
	...
	return {'list':listall,'count':listcount}
#读数据库留言
def getchatlist(frompageCount=0,limitNum=10,company_id='',sendtype=''):
	sqlarg=''
	argument=[]
...
def getfavoritelist(frompageCount,limitNum,company_id,checkStatus):
	...
	if  str(checkStatus)=='1' :
		sqlarg=' and favorite_type_code in (10091006,10091000,10091001,10091007)'
	if  checkStatus=='2':
		sqlarg=" and favorite_type_code in (10091002,10091003) "
	if   checkStatus=='3':
		sqlarg=" and favorite_type_code=10091004"
	if  checkStatus=='4':
		sqlarg=" and favorite_type_code=10091005"
	if  checkStatus=='5':
		sqlarg=" and favorite_type_code=10091012"
	
	sql="select count(0) from myfavorite where company_id=%s  "+sqlarg
	alist=dbc.fetchnumberdb(sql,[company_id])
	if alist:
		listcount=alist
	print "sqlarg:",sqlarg
	sql2="select id,favorite_type_code,content_id,content_title,gmt_created from myfavorite where company_id="+str(company_id)+" "+sqlarg+" order by gmt_created desc limit "+str(frompageCount)+","+str(limitNum)
	print sql2
	...
	if alist:
		for list in aalist:
			...
			gmt_created=list[4]
			gmt_created=formattime(gmt_created,2)
			favorite_url=None
			favorite_urls=None
			favorite_type=None
			...
			lista={'favid':favid,'favorite_type_code':favorite_type_code,'favorite_type':favorite_type,'favorite_url':favorite_url,'favorite_urls':favorite_urls,'content_id':content_id,'content_title':content_title,'gmt_created':gmt_created}
...
def getmyproductsbyid(id):
	...
	if list:
		if not list[4]:
			...
			price=list[4]
		alist={'id':list[0],'title':list[1],'quantity':list[2],'quantity_unit':list[3],'price':price,'price_unit':list[5],'details':list[6],'categorycode':list[7],'products_type_code':list[8]}
		return alist
#----供求属性
def getproducts_property(id):
	sql="select source,origin,specification,impurity,color,useful,appearance,manufacture from products where id=%s"
	productlist=dbc.fetchonedb(sql,[id])
	listmore=[]
	if productlist:
		#供求更多属性
		...
		if resultmore:
			for ll in resultmore:
				...
				if "交易说明" in ll[0]:
					fildname="tradesm"
				list={'property':ll[0],'content':contentp,'fildname':fildname}
				listmore.append(list)
		source=productlist[0]
		listmore=getmoreproperty(listmore,'货源地',source,'source')
		origin=productlist[1]
		listmore=getmoreproperty(listmore,'来源产品',origin,'origin')
		specification=productlist[2]
		listmore=getmoreproperty(listmore,'产品规格',specification,'specification')
		impurity=productlist[3]
		listmore=getmoreproperty(listmore,'杂质含量',impurity,'impurity')
		color=productlist[4]
		listmore=getmoreproperty(listmore,'颜色',color,'color')
		useful=productlist[5]
		listmore=getmoreproperty(listmore,'用途',useful,'useful')
		appearance=productlist[6]
		listmore=getmoreproperty(listmore,'外观',appearance,'appearance')
		manufacture=productlist[7]
		listmore=getmoreproperty(listmore,'加工说明',manufacture,'manufacture')
	return listmore
#----供求过期时间
def getproducts_expire(proid):
	sql="select day from products_expire where products_id=%s"
...
def getcategory_products(code):
	...
	if list:
		...
		return alist
def getcategorylabel_fromcode(code):
	sql="select label from category where code=%s"
	results = dbc.fetchonedb(sql,[code])
	if results:
		return results[0]
#保存定制
