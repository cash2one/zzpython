...
class ztrade:
    ...
    def getcompanyproductslist(self,frompageCount,limitNum,kname='',company_id='',pdt_type=''):
        ...
        return {'list':listall,'count':listcount}
    #公司供求信息 翻页
    def getcompanyproductslistnew(self,kname="",frompageCount="",limitNum="",company_id="",pdt_type="",status="",realdata=""):
        #黑名单客户
        if company_id:
...
#                    title=getlightkeywords(cl,[title],kname,"offersearch_new")
    ...
    def getproductdetail(self,id):
        ...
        sql=sql+" from products where id=%s and (is_pause=0 and check_status in (1,0) and is_del=0)"
        ...
        if plist:
            ...
            expire_time=None
            sqlc="select day from products_expire where products_id=%s"
            exlist=self.dbc.fetchonedb(sqlc,[id])
            if exlist:
                expire_day=exlist[0]
                if str(expire_day)=="365":
                    expire_time="长期有效"
                else:
                    expire_time=str(expire_day)+"天"
                today=datetime.date.today()
                
                postdate=formattime(plist[20],1)
                format="%Y-%m-%d";
                postdate=strtodatetime(postdate,format)
                
                oneday=datetime.timedelta(days=int(expire_day))   
                expireday=postdate+oneday
                
                if date_to_int(expireday)<date_to_int(today):
...
#                    company_numb=self.getpricelist_company_count(title)
    ...
    def joinaddressbook(self,company_id,forcompany_id):
        ...
        if not result:
            ...
            self.dbc.updatetodb(sqlc,[company_id,forcompany_id,gmt_created])
            return 1
        else:
            return None
    #----类别列表
