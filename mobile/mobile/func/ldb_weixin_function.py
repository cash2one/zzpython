...
#            cl.SetFilter('apply_status',[1])
    ...
    def getldbbalance(self,company_id):
        mousefirst=getToday().strftime( '%-Y-%-m-1')
    ...
    def getphonerecords(self,frompageCount,limitNum,company_id,datearg=''):
        ...
        if sum_call_fee>0:
            sql='select caller_id,call_fee,start_time,end_time,state,id,province from phone_log where company_id=%s '+sqlargall+' order by gmt_created desc limit '+str(frompageCount)+','+str(limitNum)
            ...
            for result in resultlist:
                ...
                if resultd:
                    state="1"
                    call_fee="未接查看10"
                list={'id':result[5],'caller_id':caller_id,'statetxt':statetxt,'caller_id_':result[0][:8]+'***','call_fee':call_fee,'start_time':formattime(result[2],1),'end_time':formattime(result[3],1),'state':state,'province':province,'isonephone':isonephone}
                listall.append(list)
    ...
    def getphoneclick(self,frompageCount,limitNum,company_id):
        ...
        for result in resultlist:
            ...
            target_id=result[0]
            list={'target_name':target_name,'click_fee':'%.2f'%result[1],'gmt_created':formattime(result[2],1),'company_id':target_id}
            listall.append(list)
